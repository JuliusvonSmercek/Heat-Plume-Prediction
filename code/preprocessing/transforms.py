...
import torch
from torch import linalg, nonzero
...
class SignedDistanceTransform:
    ...
    def __call__(self, data: dict):
        print("Start SignedDistanceTransform")

        # check if Material ID is in data (inputs vs. labels)
        if "Material ID" not in data.keys():
            print("No material ID in data, no SignedDistanceTransform")
            return data

        def get_loc_hp(): # TODO clean up - this appearsquite often in various forms
            if "Material ID" in data.keys():
                loc_hp = nonzero(data["Material ID"] == torch.max(
                    data["Material ID"]))
                if loc_hp.numel() == 0:
                    return None
                if loc_hp.shape[0] > 1:
                    print(f"loc_hp returns more than one position: {loc_hp.shape[0]}")
                print(f"loc_hp: {loc_hp}")
                return loc_hp

        loc_hp = get_loc_hp()
        if loc_hp is None:
            print("No hp location found, skipping SignedDistanceTransform")
            return data
        print(f"Keys in data: {data.keys()}")
        data["SDF"] = self.sdf(data["SDF"].float(), loc_hp.float())
        print("SignedDistanceTransform done")
        return data
...
def get_transforms(reduce_to_2D: bool = True, inputs=None):
    ...
    print(f"Inputs for get_transforms: {inputs}")
    if inputs:
