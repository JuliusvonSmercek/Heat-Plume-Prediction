...
def preprocessing(args:dict):
    ...
    network = args.get("network", "rnn").lower()
    if is_unprepared(args["data_prep"]):
        if network in ["convlstm", "rnn", "lstm"]:
            info = prepare_dataset_for_sequence(args, info=None)
            print("Preparing dataset...")
        else:
            info = prepare_dataset(args, info=None)
    else:
...
def is_unprepared(path:Path):
    ...
    result = is_empty(path / "Inputs") or is_empty(path / "Labels") or not (path / "info.yaml").exists()
...
def prepare_dataset_for_sequence(args:dict, info:dict = None):
    transforms = get_transforms(inputs=args["inputs"])
    print("Transforms: ", [transform.__class__.__name__ for transform in transforms.transforms])
    ...
    for data_path, run in tqdm(zip(data_paths, runs), desc="Converting", total=total):
        ...
        if "t" in args.get("inputs", "") and args.get("previous_results") is not None:
            ...
            if prediction_file.exists():
                ...
                if predicted_t.ndim == 2:
                    predicted_t = predicted_t.unsqueeze(0)
                print(f"Shape of predicted_t: {predicted_t.size()}")
                x["Temperature [C]"] = predicted_t
            else:
                raise FileNotFoundError(f"Step3 prediction not found: {prediction_file}")
        ...
        for time in times_str:
            for output in outputs:
                print(f"times_str[1:]: {times_str[1:]}")
                value = load.load_raw_data(data_path, time, [output], dims, times_str[1:], print_bool=print_bool)[output]
        ...
        if info is None: calc.add_data(y)
        y = tensor_transform(y)
        print("x shape in prepare:", x.shape)
        print("y shape in prepare:", y.shape)
...
def expand_property_names(properties: str):
    translation = {
        ...
        "1": "Streamline-Sum_Position", # a  f
        "2": "Streamline-Sum_RelativeUncertainty", # b, g
        "3": "Streamline-Sum_TimeFaded-Position", # c h
        "4": "Streamline-Max_TimeFaded", 
        "5": "Streamline-Sum_TimeSeasons-Position", # d i
        "6": "Streamline-Max_TimeSeasons",
        "7": "Streamline-TemperatureApproximation", # e j
    }
