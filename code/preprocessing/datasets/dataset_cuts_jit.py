...
class SimulationDatasetCutsSequential(SimulationDatasetCuts):
    ...
    def __init__(self, path:Path, time_steps_to_predict, max_simulation_timestep:int, skip_per_dir:int=4, box_size:int=64, ids:List[int]=[0,], case:str="train", log_path: Path | None = None, ratio_blank_boxes: float = 0.1):
        DatasetBasis.__init__(self, path, box_size)
        ...
        assert self.inputs.dim() == self.labels.dim() == 5, f"inputs and labels should be 5D (channels, datapoints, time, H,W), but got {self.inputs.shape} and {self.labels.shape}"
        
        self.spatial_size = self.inputs.shape[-2:]
        self.n_dp = self.inputs.shape[1]
        if not self.inputs.shape[-2:] == self.labels.shape[-2:]:
            raise ValueError(f"Spatial size of inputs {self.inputs.shape[-2:]} and labels {self.labels.shape[-2:]} do not match. See SimulationDatasetCut to fix.")
    ...
    def _save_active_tile_label(self, labels: torch.Tensor, id: int, y0: int, x0: int, debug_dir: str = "debug_tiles_dataset"):
