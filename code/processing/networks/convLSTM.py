...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, activation, frame_size, conv_features, kernel_sizes,
                 norm_type="group", groupnorm_groups=8):
    ...
    def set_activation_logger(self, writer, log_every: int = 100):
        ...
        print("[gate-log] writer set:", self.writer is not None, "log_every", self.log_every)
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, activation, frame_size, prev_boxes, extend, 
                 conv_features, kernel_sizes, norm_type="group", groupnorm_groups=8, tbptt_k=2):
...
class Seq2Seq(nn.Module):
    ...
    groupnorm_groups=8):
        ...
        self.initial_H_projections = nn.ModuleList([
            nn.Conv2d(1, enc_conv_features[-1], kernel_size=1)
        ...
        self.sequential.add_module(
            "convlstm1", ConvLSTM(
                in_channels=static_features, out_channels=enc_conv_features[-1],  
                ...
                norm_type=norm_type, groupnorm_groups=groupnorm_groups)
        )
        ...
        self.static_encoder = nn.Sequential(
            Seq2Seq._block(self.static_channels, static_features, kernel_size=3,
                        norm_type=norm_type, groupnorm_groups=groupnorm_groups),
            Seq2Seq._block(static_features, static_features, kernel_size=3,
                        norm_type=norm_type, groupnorm_groups=groupnorm_groups),
        )
    ...
    def forward(self, X, init_frame=None):
        ...
        for i in range(self.num_layers):
            X, encoded_features = self.sequential[i*2](X, hidden_H[i], hidden_C[i], self.film_net, scalars) # ConvLSTM layer
            B, C, T, H, W = X.shape
    ...
    def _block(in_channels, features, kernel_size=5, norm_type="group", groupnorm_groups=8):
        ...
        class ResBlock(nn.Module):
            def __init__(self):
                super().__init__()
                self.conv1 = nn.Conv2d(in_channels, features, kernel_size,
                ...
                self.conv3 = nn.Conv2d(features, features, kernel_size,
                                    padding="same", bias=True)
                self.act3  = nn.LeakyReLU(0.1, inplace=True)
                self.skip  = nn.Conv2d(in_channels, features, kernel_size=1, bias=False) \
    ...
    @staticmethod
    def _dilated_block(in_channels, features, kernel_size=3, dilation=1,
                   norm_type="group", groupnorm_groups=8):
        if norm_type == "group":
            g = _safe_group_count(features, groupnorm_groups)
            norm_layer = nn.GroupNorm(num_groups=g, num_channels=features)
    ...
    def calculate_receptive_field(self) -> dict:
        ...
        static_end = sum(
            1 for label, _, _, _ in ops if label.startswith("static")
        )
        ...
        for idx, row in enumerate(rows):
            if idx == static_end:
                print(f"  -- cell encoder (per timestep) --")
            print(
        ...
        print(f"{'Static encoder RF':<25} {static_rf:>6}")
        print(f"{'Total RF':<25} {r:>6}  (input size: {cell.frame_size[0]}x{cell.frame_size[1]})")
        print(f"RF covers {100*r/cell.frame_size[0]:.1f}% of the input width\n")
