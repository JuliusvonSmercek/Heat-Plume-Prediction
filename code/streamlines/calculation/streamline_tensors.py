...
from utils.yaml_parser import convert_injection_config
...
def draw_batched_polylines(
  ...
  Args:
    ...
    seasonal_density_accumulator: Global heatmap for seasonal flow (H, W).
    ...
    weights_seasonal: 1D Time-dependent weights for seasonal density.
  ...
  total_seasonal = torch.zeros_like(seasonal_density_accumulator)
  ...
  for b_start in range(0, num_samples, BATCH_CHUNK_SIZE):
    ...
    segment_indices = torch.bucketize(
      global_pixel_indices, segment_boundaries, right=True
    )

    # Calculate interpolation factor 't' [0, 1]
    segment_starts_flat = (
      segment_boundaries[segment_indices] - steps[segment_indices]
    )
    t_factor = (
      global_pixel_indices - segment_starts_flat
    ).float() * inv_steps[segment_indices]

    # --- 3. Geometry Interpolation ---
    # p(t) = p0 + (p1 - p0) * t
    interpolated_coords = (
      segment_start[segment_indices]
      + delta[segment_indices] * t_factor.unsqueeze(1)
    )
    ...
    w_seas_start = weights_seasonal[:-1].repeat(current_batch_size)
    w_seas_end = weights_seasonal[1:].repeat(current_batch_size)

    # Stack channels: 0=Normal, 1=Faded, 2=Seasonal
    w_start = torch.stack([w_norm_start, w_faded_start, w_seas_start], dim=1)
    w_end = torch.stack([w_norm_end, w_faded_end, w_seas_end], dim=1)

    interpolated_weights = (
      w_start[segment_indices]
    ...
    ).repeat_interleave(num_segments)
    ...
    x_neighbors = torch.stack(
      [x_floor, x_floor + 1, x_floor, x_floor + 1], dim=1
    ).view(-1)
    y_neighbors = torch.stack(
      [y_floor, y_floor, y_floor + 1, y_floor + 1], dim=1
    ).view(-1)

    # Bounds check
    valid_mask = (
      (x_neighbors >= 0)
      & (x_neighbors < canvas_width)
      & (y_neighbors >= 0)
      & (y_neighbors < canvas_height)
    )
    ...
    values_expanded = (
      interpolated_weights.unsqueeze(1)
      .expand(-1, 4, -1)
      .reshape(-1, 3)
    )
    ...
    batch_ids_expanded = (
      pixel_batch_ids.unsqueeze(1).expand(-1, 4).reshape(-1)
    )
    ...
    unique_hashes, inverse_indices = torch.unique_consecutive(
      unique_pixel_hash, return_inverse=True
    )

    # Reduce Step 1: Maximize within the same line
    reduced_values = torch.zeros(
      (unique_hashes.shape[0], 3),
      device=device,
    ...
    total_density.view(-1).scatter_add_(
      0, global_spatial_indices, reduced_values[:, 0]
    )
    total_density_sq.view(-1).scatter_add_(
      0, global_spatial_indices, reduced_values[:, 0].pow(2)
    )
    total_faded.view(-1).scatter_add_(
      0, global_spatial_indices, reduced_values[:, 1]
    )
    total_seasonal.view(-1).scatter_add_(
      0, global_spatial_indices, reduced_values[:, 2]
    )
  ...
  epsilon = 1e-6
  uncertainty_relative = uncertainty_absolute / (total_density + epsilon)

  # Update global accumulators using MAX blending
  density_accumulator.view(-1).copy_(
    torch.maximum(density_accumulator.view(-1), total_density.view(-1))
  )
  faded_density_accumulator.view(-1).copy_(
    torch.maximum(faded_density_accumulator.view(-1), total_faded.view(-1))
  )
  seasonal_density_accumulator.view(-1).copy_(
    torch.maximum(seasonal_density_accumulator.view(-1), total_seasonal.view(-1))
  )
  variance_accumulator.view(-1).copy_(
    torch.maximum(variance_accumulator.view(-1), uncertainty_absolute.view(-1))
  )
...
def make_streamlines_gpu(
  ...
  with torch.inference_mode():
    ...
    density_acc = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    faded_density_acc = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    time_max_acc = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    seasonal_max_acc = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    seasonal_density_acc = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    variance_accumulator = torch.zeros(
      (grid_height, grid_width), device=device, dtype=torch.float32
    )
    ...
    steps_indices = torch.arange(
      0, streamline_config.steps, device=device, dtype=torch.float32
    )
    ...
    (injectionRate, seasonalCycleSteps) = convert_injection_config(
      physical_parameters.injection_rate_m3_per_s,
      streamline_config.steps,
      physical_parameters.duration_years,
      device
    )
    weights_seasonal_global = weights_seasonal_global * injectionRate / injectionRate.max()
    ...
    for b_idx in tqdm(
      ...
      current_positions = batch_positions.repeat_interleave(
        streamline_config.samples, dim=0
      )
    ...
    for key, vec in check_list.items():
      if (vec < 0).any():
        raise ValueError("Output map {} contains Negative values".format(key))
      if torch.isnan(vec).any():
        raise ValueError("Output map {} contains NaN values".format(key))
      if torch.isinf(vec).any():
        raise ValueError("Output map {} contains Inf values".format(key))
