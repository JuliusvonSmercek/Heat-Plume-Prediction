...
def sample_velocity_bilinear(
  ...
  vSample_m_per_year = F.grid_sample(fieldTensor_m_per_year, grid, mode="bilinear", padding_mode="border", align_corners=True)
...
def rwpt_seasonal_stream_kernel(
  accumEnergyGrid_flat: torch.Tensor,  # [IN/OUT] Flat grid to accumulate hits
  positions_px: torch.Tensor,
  origins_px: torch.Tensor,
  fieldTensor_m_per_year: torch.Tensor,
  injectionRates_m3_per_s: torch.Tensor, # (N_Sources, Cycle_Steps)
  ...
  # Pre-calculate Max Rate per source to normalize the "Strobe" intensity
  max_rates, _ = torch.max(injectionRates_m3_per_s, dim=1, keepdim=True)
  max_rates = max_rates.clamp(min=1e-12)

  for i in range(steps_count):
    # Background Velocity (px/s)
    qBackground_px_per_s = sample_velocity_bilinear(positions_px, fieldTensor_m_per_year, invWidth_per_px, invHeight_per_px, resolution_m_per_px, secondsPerYear_s)
    ...
    diffusionL_px2_per_s = ((molecularDiffusion_m2_per_s + alphaL_m * vSeepageMag_px_per_s) / retardationFactor_dimless) / resolution_m_per_px
    diffusionT_px2_per_s = ((molecularDiffusion_m2_per_s + alphaT_m * vSeepageMag_px_per_s) / retardationFactor_dimless) / resolution_m_per_px
    ...
    current_source_rate = currentQ_m3_per_s.squeeze()
    max_source_rate = max_rates[sourceIndices_idx].squeeze()
    strobe_scale = (current_source_rate / max_source_rate)
    
    # Draw mask
    draw_mask = active_sim_mask & (strobe_scale > 1e-5)

    if draw_mask.any():
      p1 = prev_pos[draw_mask]
      p2 = positions_px[draw_mask]
      # Scale the weight by the current seasonal intensity
      w_draw = weights[draw_mask] * strobe_scale[draw_mask]
      ...
      valid_map = (valid_x >= 0) & (valid_x < width_px) & \
                  (valid_y >= 0) & (valid_y < grid_h)
...
def generate_physical_plumes(
  ...
  with torch.inference_mode():
    ...
    for bIdx in tqdm(range(0, numHps_count, hpBatchSize_count), "Direct Solver"):
      ...
      srcIndices = torch.arange(currBatchSize, device=device).unsqueeze(1).expand(-1, config.samplesPerSource_count)

      # Lookup Weights (Rate * DeltaT) at birth time
      pRates = config.injectionRate_m3_per_s[bIdx:bEnd].gather(1, birthIndices)
      pTemps = config.injectionTemp_C[bIdx:bEnd].gather(1, birthIndices)
      pWeights = pRates * (pTemps - config.ambientTemp_C)

      # Flatten for Kernel
      flat_birth = birthIndices.flatten()
      flat_src = srcIndices.flatten()
      flat_weights = pWeights.flatten()
...
def direct_solve(
  ...
  (rate, seasonalCycleSteps1) = convert_injection_config(modeConstants.injection_rate_m3_per_s, modeDirectSolver.steps, modeConstants.duration_years, \
    heatPumpPositions_px.device)
  (temp, seasonalCycleSteps2) = convert_injection_config(modeConstants.injection_temperature_C, modeDirectSolver.steps, modeConstants.duration_years, \
    heatPumpPositions_px.device)
  ...
  # --- EXECUTE ---
  tMap_normalized = generate_physical_plumes(
    simConfig,
    heatPumpPositions_px,
    vx_m_per_year,
    vy_m_per_year,
    dims_px
  )
  ...
  debug = False
  if debug:
    info("DEBUG MODE: Cropping and Rescaling Output")
    x = 0
    tMap_normalized = tMap_normalized[x:1000 - x,x:1000 - x]
    tMap_normalized = F.interpolate(tMap_normalized.unsqueeze(0).unsqueeze(0), size=(1000, 1000), mode='bilinear', align_corners=False).squeeze()
  else:
    info("Release Mode: Rescaling Output to [0, 1]")
    temp_diff = 5.0
    tMap_normalized = (tMap_normalized - (modeConstants.ambient_temperature_C - temp_diff)) / (temp_diff * 2)
  info(f"Min: {tMap_normalized.min():.2f}, Max: {tMap_normalized.max():.2f}, Mean: {tMap_normalized.mean():.4f}")
