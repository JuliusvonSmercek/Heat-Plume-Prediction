...
from postprocessing.visualization import DataToVisualize, plot_datapoint
from preprocessing.preparing_datasets.raw_data_loading import get_hp_location_raw_np
...
def load_velocity_field(
    ...
    if use_model and model is not None:
        ...
        with torch.no_grad():
            velocity = model(data_in.unsqueeze(0)).detach().squeeze(0)
    else:
...
def extract_heat_pump_positions(
...
def save_result(
    ...
    debug(f"Saved processed datapoint: {datapoint_path}")
...
def run_visualization(
    ...
    for key, tensor_data in streamlines.items():
        ...
        plot_datapoint(
            ...
            datapoint=DataToVisualize(tensor_data, 'Input', prop_name, resolution),
            name_pic=str(output_name),
            settings_pic={'format': 'png', 'dpi': 300},
            only_inner=False,
            remove_axis=False,
            dark_mode=False
        )
